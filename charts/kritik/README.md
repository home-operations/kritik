...
`runner.image` at the release's `-tools` tag, which adds static builds of
all three. Every host `curl` reaches must pass the gateway, so add the
...
so a command cannot read the runner's credentials from `/proc`.
